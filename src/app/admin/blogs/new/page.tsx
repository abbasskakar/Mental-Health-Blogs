...
import {
  Save, Eye, ArrowLeft, Image as ImageIcon, Tag, Globe,
  Clock, Star, AlignLeft, Plus, X, CheckCircle, Upload, Loader2,
} from "lucide-react";
...
export default function NewBlogPage() {
  ...
  const handleTitleChange = (title: string) => {
    setForm((f) => ({ ...f, title, slug: slugify(title), meta_title: title }));
  };
    ...
    <div className="text-body min-h-screen">
      ...
      <div className="bg-surface border border-line rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base sm:text-lg font-bold text-heading">New Blog Post</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => handleSave("published")} disabled={saving} className={cn("px-4 py-2 rounded-xl text-sm font-semibold flex items-center gap-1.5 transition-all shadow-sm", saved ? "bg-accent text-white" : "bg-accent hover:bg-accent-hover text-white disabled:opacity-50")}>
            {saving ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /><span className="hidden sm:inline">Publishing...</span></>
              : saved ? <><CheckCircle className="w-3.5 h-3.5" /><span className="hidden sm:inline">Published!</span></>
              : <><Globe className="w-3.5 h-3.5" /><span className="hidden sm:inline">Publish Now</span><span className="sm:hidden">Publish</span></>
            }
          </button>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {activeTab === "write" ? (
            <div className="bg-surface border border-line rounded-2xl overflow-hidden">
              ...
              <div className="p-6 border-b border-line">
                <input
                  ...
                  className="w-full text-2xl font-bold text-heading bg-transparent outline-none placeholder-faint resize-none"
                />
                <p className="text-xs text-faint mt-2">Slug: /blog/<span className="text-accent">{form.slug || "your-blog-slug"}</span></p>
              </div>
              ...
              <div className="px-6 py-4 border-b border-line">
                ...
                <textarea
                  ...
                  placeholder="Write a short summary that appears in blog cards and search results..."
                  rows={2}
              ...
              <div className="p-6">
                <textarea
                  ...
                  placeholder={`# Start writing your blog post here...\n\nYou can use Markdown syntax:\n- **Bold text** and *italic text*\n- # Heading 1, ## Heading 2\n- > Blockquote\n- [Link](https://example.com)\n- ![Image alt](https://image-url.com)\n\nShare evidence-based mental health insights that will help your readers...`}
                  rows={24}
            ...
            <div className="bg-surface border border-line rounded-2xl p-8">
              <h1 className="text-3xl font-bold text-heading mb-4">{form.title || "Untitled Blog Post"}</h1>
          ...
          <div className="bg-surface border border-line rounded-2xl p-6">
            <h3 className="font-bold text-heading mb-4 flex items-center gap-2">
              <Globe className="w-4 h-4 text-accent" /> SEO Settings
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-faint uppercase tracking-wide mb-2">Meta Title <span className="text-faint normal-case font-normal">({(form.meta_title || form.title).length}/60 chars)</span></label>
                <input value={form.meta_title || form.title} onChange={(e) => setForm({ ...form, meta_title: e.target.value })} maxLength={60} placeholder="SEO Title (leave empty to use blog title)" className="w-full px-4 py-3 bg-surface-alt border border-line text-body rounded-xl text-sm outline-none focus:border-accent transition-colors" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-faint uppercase tracking-wide mb-2">Meta Description <span className="text-faint normal-case font-normal">({form.meta_description.length}/160 chars)</span></label>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-line rounded-2xl p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between py-2">
                ...
                <button onClick={() => setForm({ ...form, is_featured: !form.is_featured })} className={cn("w-11 h-6 rounded-full flex items-center transition-colors", form.is_featured ? "bg-accent" : "bg-surface-alt")}>
          ...
          <div className="bg-surface border border-line rounded-2xl p-5">
            ...
            <div className="space-y-1.5">
              {categories.length === 0 && (
                <p className="text-xs text-faint">Loading categories...</p>
              )}
              {categories.map((cat) => (
                <button
                  key={cat.id}
          ...
          <div className="bg-surface border border-line rounded-2xl p-5">
            ...
            <div className="flex gap-2 mb-3">
              <input
                ...
                className="flex-1 px-3 py-2 bg-surface-alt border border-line text-body placeholder-faint rounded-lg text-xs outline-none focus:border-accent"
              />
              <button onClick={addTag} className="p-2 rounded-lg bg-accent-subtle text-accent hover:opacity-80 transition-colors">
          ...
          <div className="bg-surface border border-line rounded-2xl p-5">
            ...
            {form.featured_image ? (
              <div className="relative rounded-xl overflow-hidden">
                ...
                <button onClick={() => setForm({ ...form, featured_image: "" })} className="absolute top-2 right-2 p-1 rounded-full bg-black/50 text-white">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ) : (
              <div>
                <div className="border-2 border-dashed border-line rounded-xl p-6 text-center hover:border-accent transition-colors cursor-pointer">
                  ...
                  <p className="text-xs text-faint">Click to upload or drag & drop</p>
                </div>
                <div className="mt-2">
                  <input value={form.featured_image} onChange={(e) => setForm({ ...form, featured_image: e.target.value })} placeholder="Or paste image URL..." className="w-full px-3 py-2 bg-surface-alt border border-line text-body placeholder-faint rounded-lg text-xs outline-none focus:border-accent" />
                </div>
              </div>
            )}
          </div>
          ...
          {/* Reading Time */}
          <div className="bg-surface border border-line rounded-2xl p-5">
            <h3 className="font-bold text-heading mb-3 text-sm flex items-center gap-2">
              <Clock className="w-4 h-4 text-faint" /> Reading Time
            </h3>
            <div className="flex items-center gap-3">
              <input type="number" min={1} max={60} value={estReadingTime} readOnly className="w-20 px-3 py-2 bg-surface-alt border border-line text-body rounded-xl text-sm outline-none text-center" />
              <span className="text-sm text-faint">minutes (auto-calculated)</span>
            </div>
          </div>
        </div>
      </div>
